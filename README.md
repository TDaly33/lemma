...
- **Enclitic Double-Accentuation**: Proparoxytone headwords and inflections (e.g. χαρτοφύλακα) get their enclitic-attached spelling (χαρτοφύλακά, as in "χαρτοφύλακά του") indexed automatically, since Kindle does not auto-generate this second-accent variant
- **Punctuation-Attached Lookup**: The headword plus its top-ranked inflections get trailing/leading punctuation variants indexed (e.g. "λέξη," or "«λέξη"), since Kindle's word-selection can include adjacent punctuation in the lookup string; tune the scope with `--punct-variants`
...
- `--punct-variants N`: Trailing/leading punctuation index variants for the headword plus its top N frequency-ranked inflections (default: 1, `0` disables). N=1 was chosen after measuring real `.mobi` size impact against a full ~31K-headword build: content/MOBI size grows roughly linearly per additional ranked form in this range (~2×/~3×/~3.7×/~6.6× the unaugmented size at N=1/3/5/20 respectively), with no economy of scale that favors a higher N, so the smallest N that still covers the headword's single most common inflection was picked as the default. Punctuation variants are resolved for cross-headword collisions dictionary-wide before rendering, the same way as the enclitic double-accent variants below. See "Punctuation-Attached Lookup" above.
